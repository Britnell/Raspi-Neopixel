/*
 *  Now with a cellular automaton, first implementation
 *
 */
...
function init_automaton(){
	...
	for(let x=0; x<seeds; x++){
		...
		if(ra<0.3){
			...
			console.log( row, row_pixels[10+row].length, ctr, row_pixels[10+row], row_pixels[10+row][ctr]  );
...
function step_automation(){
	...
	if(no_change_counter==6)	reset_automaton = true;

	// * count generations
	generations++;
	console.log(' gernerations ', generations);
	if(generations==30){
		cell_rule = random_rule(3);	  //	( r()<0.5 ? random_rule(2) : random_rule(3)  );
		generations = 0;
